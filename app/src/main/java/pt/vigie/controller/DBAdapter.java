...
public class DBAdapter {
    ...
    public boolean hasLogin(){
        Cursor cursor = database.rawQuery("SELECT count(id) AS count FROM " + USER_TABLE + " WHERE is_logged = 1", null);

        cursor.moveToFirst();

        if(cursor.getColumnCount() > 0 && cursor.getInt(cursor.getColumnIndex("count")) > 0)
            return true;
        else
            return false;
    }
    ...
    public String getUserEmail(){
        Cursor cursor = database.rawQuery("SELECT email FROM " + USER_TABLE + " WHERE is_logged = 1", null);
    ...
    public String getUserPassword(){
        Cursor cursor = database.rawQuery("SELECT password FROM " + USER_TABLE + " WHERE is_logged = 1", null);
    ...
    public long insertUser(String email, String password){
        ...
        long insertId = database.insert(USER_TABLE, null, values);
    ...
    {
        Cursor cursor = database.rawQuery("DELETE FROM " + USER_TABLE, null);
    }
